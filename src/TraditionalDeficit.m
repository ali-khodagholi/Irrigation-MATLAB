...
while(Time(i-1) < crop.T)
    ...
    if (s<soil.s_tild)
        ...
        ET_total = ET_total + ET(soil.moisture(i),soil,crop)*0.25;
        ET_a(i) = ET(soil.moisture(i),soil,crop);
        
        i = i + 1;
        
        Time(i) = Time(i-1);
        soil.moisture(i) = soil.s_hat;
        ET_total = ET_total + ET(soil.moisture(i),soil,crop)*0.25;
        ET_a(i) = ET(soil.moisture(i),soil,crop);
        ...
        Time(i) = Time(i-3) + 1;
        soil.moisture(i) = soilmoisture(soil.moisture(i-1),soil,crop);
        ET_total = ET_total + ET(soil.moisture(i),soil,crop)*0.5;
        ET_a(i) = ET(soil.moisture(i),soil,crop);
        
        % Calculate Value of Irrigation
        Irrigation(Time(i)) = (soil.s_hat - soil.s_tild)*soil.phi*crop.Zr;
    else
        soil.moisture(i) = s;
...
grid on;
title('Actual ET - Traditional: Deficit Irrigation');
xlabel('Time (Day)');
...
title('Soil Moisture - Traditional: Deficit Irrigation');
grid on;
hold on;
xlabel('Time (Day)');
ylabel('Soil Moisture');
axis ([1 crop.T 0.3 0.7]);
plot(1:crop.T,linspace(soil.s_star,soil.s_star,crop.T),'--k');
plot(1:crop.T,linspace(soil.s_fc,soil.s_fc,crop.T),'-.k');
hold off;

figure;
plot(1:crop.T,Irrigation,'k');
title('Irrigation - Traditional: Deficit Irrigation');
grid on;
xlabel('Time (Day)');
